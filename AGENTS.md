...
1차 독자: **개발자/기획자/PM 등 의사결정·논의를 다루는 모든 직군** — 도메인 무관. 본인의 토론 컨텍스트를 시스템 프롬프트로 자유 정의하므로 게임·SaaS·교육·연구 어느 쪽이든 그대로 동작한다.

차별화 한 줄: **"사용자가 AI들의 토론에 끼어들 수 있는 도구"**. 단순 다중 호출이 아니라 **개입 가능성** 과 **직렬 라운드 + AbortSignal 분리** 가 핵심 포지셔닝.
...
2. **운영 통찰** (차별화 핵심, 형식 가드레일):
   - 분량 300~500자.
   - 강제 포함 3개 항목:
     1. 왜 표준 OAuth를 구현하지 않았는가 (Anthropic·OpenAI provider 부재 → CLI 모드가 사실상의 OAuth 대체, BYOK 모델로 대체)
   ...
   - "이게 진짜 통찰인가" 한 번에 판정 가능한 구조 우선.

데모 시나리오: **도메인 무관**. 사용자가 자기 도메인을 시스템 프롬프트와 토론 주제로 정의한다. UI 좌패널 프리셋 3종(요구사항 정리·의사결정 비교·글 다듬기)은 다양한 직군이 본인 일을 즉시 연상하도록 한 마중물이며, 핵심 데모는 "사용자가 토론 중 제약을 끼워넣어 토론 흐름이 재정렬되는 모습"을 보여주는 것.

본 프로젝트의 이력: 베이글코드 신작팀 AI 개발자 채용 과제 제출본(`v0.1.0-bagelcode-submission` 태그)에서 출발했으며, 이후 범용 멀티 AI 토론 도구로 재포지셔닝됨. 채용 과제 시점 산출물은 동결 태그·`docs/legacy/`로 보존되어 main 워크플로엔 영향 없음.
